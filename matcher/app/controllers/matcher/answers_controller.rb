module Matcher
  class AnswersController < ApplicationController
    ...
    def new
      ...
      if current_user.profile.answers.where(matcher_question_id: @question.id).count > 0
        redirect_to new_answer_path and return
      end

      @answer = @question.answers.build(matcher_profile_id: current_user.profile.id, axis: axis)
    end

    def create
      @profile = current_user.profile

      question_id = params[:matcher_answer][:matcher_question_id]
      axis = params[:matcher_answer][:axis]
      parity = params[:matcher_answer][:parity].to_i
      value = params[:matcher_answer][:value].to_i * parity
      
      @answer = @profile.answers.build({
        matcher_question_id: question_id,
        axis: axis,
        value: value
      })
